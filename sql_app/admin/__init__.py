...
class UserAdmin(ModelView, model=User):
    ...
    form_excluded_columns = [
        ...
        User.audit_logs,
        User.approvals,
        User.created_blacklist_entries,
        User.removed_blacklist_entries,
        User.notifications,
        User.requests,
    ]
...
class RequestPersonAdmin(ModelView, model=RequestPerson):
    ...
    column_list = [
        ...
        RequestPerson.company,
        RequestPerson.status,
        RequestPerson.nationality,
    ]
    column_searchable_list = [
        RequestPerson.firstname,
        RequestPerson.lastname,
        RequestPerson.iin,
        RequestPerson.doc_number,
    ]
    column_filters = [
...
class RelativeDateFilter:
    ...
    async def get_filtered_query(
        ...
        return query.filter(and_(column_obj >= start, column_obj < end))


class AuditLogAdmin(ModelView, model=AuditLog):
    """Админка для журнала действий"""

    column_list = [AuditLog.id, AuditLog.entity, AuditLog.action, AuditLog.timestamp]
...
class VisitLogAdmin(ModelView, model=VisitLog):
    ...
    column_list = [
        VisitLog.id,
        VisitLog.request_person_id,
        VisitLog.checkpoint_id,
        VisitLog.check_in_time,
        VisitLog.check_out_time,
    ]
    column_filters = [
        ForeignKeyFilter(
            VisitLog.checkpoint_id, Checkpoint.name, foreign_model=Checkpoint
        ),
