...
async def get_current_user_for_cp_router(token: str = Depends(oauth2_scheme_cp), db: Session = Depends(get_db)) -> models.User:
    ...
    if not SECRET_KEY or not ALGORITHM:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server auth configuration error (cp router)")
    try:
...
async def get_current_active_user_for_cp_router(current_user: models.User = Depends(get_current_user_for_cp_router)) -> models.User:
    if not current_user.is_active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Inactive user (cp router)")
    return current_user
...
async def read_checkpoint_requests(
    ...
    if status_filter:
        print(f"Note: status_filter ('{status_filter.value}') provided but this endpoint uses fixed statuses by default for checkpoint operators.")
        # Potentially, could pass status_filter to crud function if it's designed to override default statuses.

    print(f"User {current_user.username} (Role: {current_user.role.code if current_user.role else 'None'}) fetching requests for checkpoint ID: {cp_id}.")
