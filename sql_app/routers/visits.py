...
async def update_visit_log_checkout(
    ...
    """
    check_permission(current_user) # This is for the original admin/checkpoint operator endpoint
...
@router.post("/entry", response_model=schemas.VisitLog, status_code=status.HTTP_201_CREATED)
async def record_visitor_entry(
    ...
    db_request = crud.get_request(db, request_id=visit_log_in.request_id, user=current_user) # Pass current_user for RBAC consistency
    if not db_request:
        ...
        db_request = db.query(models.Request).filter(models.Request.id == visit_log_in.request_id).first()
        if not db_request:
    ...
    existing_log = db.query(models.VisitLog).filter(
...
async def record_visitor_exit(
    ...
    if updated_log is None: # Should not happen if db_visit_log was found
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update visit log.")

    print(f"[INFO] KPP User {current_user.username} (ID: {current_user.id}) recorded EXIT for VisitLog ID: {updated_log.id}, RequestPerson ID: {updated_log.request_person_id}")
    return updated_log
