...
from sql_app import rbac, models, crud # Assuming crud is needed for get_department_descendant_ids
...
@pytest.mark.parametrize("role_code, expected", [
...
def test_can_access_dept_history_is_dept_head_own_dept(db_session_mock, mock_user_builder):
    ...
    assert rbac.can_user_access_visit_log_department_history(db_session_mock, user, request_creator_dept_id) == True

@patch('sql_app.crud.get_department_descendant_ids')
def test_can_access_dept_history_is_dept_head_descendant_dept(mock_get_descendants, db_session_mock, mock_user_builder):
    user = mock_user_builder(1, DEPARTMENT_HEAD_ROLE_CODE, department_id=10)
...
@patch('sql_app.crud.get_department_descendant_ids')
def test_can_access_dept_history_is_dept_head_unrelated_dept(mock_get_descendants, db_session_mock, mock_user_builder):
    ...
    assert rbac.can_user_access_visit_log_department_history(db_session_mock, user, request_creator_dept_id) == False
    mock_get_descendants.assert_called_once_with(db_session_mock, 10)

def test_can_access_dept_history_not_dept_head(db_session_mock, mock_user_builder):
    user = mock_user_builder(1, ADMIN_ROLE_CODE, department_id=10) # Admin, not dept head
    request_creator_dept_id = 10
    assert rbac.can_user_access_visit_log_department_history(db_session_mock, user, request_creator_dept_id) == False

def test_can_access_dept_history_no_user_dept(db_session_mock, mock_user_builder):
    user = mock_user_builder(1, DEPARTMENT_HEAD_ROLE_CODE, department_id=None) # Dept head with no dept
    request_creator_dept_id = 10
    assert rbac.can_user_access_visit_log_department_history(db_session_mock, user, request_creator_dept_id) == False
...
def test_can_access_div_history_is_div_manager_own_div_dept(db_session_mock, mock_user_builder):
    # User is Div Manager, their dept is type DIVISION, request from same dept
    user = mock_user_builder(1, DIVISION_MANAGER_ROLE_CODE, department_id=5, department_type=DepartmentType.DIVISION)
    request_creator_dept_id = 5
    assert rbac.can_user_access_visit_log_division_history(db_session_mock, user, request_creator_dept_id) == True

def test_can_access_div_history_is_div_manager_dept_not_division(db_session_mock, mock_user_builder):
    # User is Div Manager, but their dept is NOT type DIVISION
    user = mock_user_builder(1, DIVISION_MANAGER_ROLE_CODE, department_id=5, department_type=DepartmentType.DEPARTMENT)
    request_creator_dept_id = 5
    assert rbac.can_user_access_visit_log_division_history(db_session_mock, user, request_creator_dept_id) == False

def test_can_access_div_history_is_div_manager_unrelated_dept(db_session_mock, mock_user_builder):
    # User is Div Manager, their dept is DIVISION, but request from different dept
    user = mock_user_builder(1, DEPUTY_DIVISION_MANAGER_ROLE_CODE, department_id=5, department_type=DepartmentType.DIVISION)
...
def test_can_access_div_history_not_div_manager(db_session_mock, mock_user_builder):
    user = mock_user_builder(1, DEPARTMENT_HEAD_ROLE_CODE, department_id=5, department_type=DepartmentType.DIVISION)
    request_creator_dept_id = 5
    assert rbac.can_user_access_visit_log_division_history(db_session_mock, user, request_creator_dept_id) == False

def test_can_access_div_history_user_no_dept_details(db_session_mock, mock_user_builder):
    ...
    request_creator_dept_id = 5
    assert rbac.can_user_access_visit_log_division_history(db_session_mock, user_no_dept, request_creator_dept_id) == False

    user_no_dept_type = mock_user_builder(1, DIVISION_MANAGER_ROLE_CODE, department_id=5, department_type=None)
    user_no_dept_type.department.type = None # Ensure department type is None
    assert rbac.can_user_access_visit_log_division_history(db_session_mock, user_no_dept_type, request_creator_dept_id) == False
...
@pytest.mark.parametrize("rbac_function_to_test", [
    rbac.can_user_access_visit_log_department_history,
    rbac.can_user_access_visit_log_division_history
])
def test_rbac_functions_user_no_role(db_session_mock, mock_user_builder, rbac_function_to_test):
    ...
    assert rbac_function_to_test(db_session_mock, user_no_role, request_creator_dept_id) == False
