...
def upgrade() -> None:
    ...
    if not inspector.has_table('request_checkpoint'):
        op.create_table(
            'request_checkpoint',
            sa.Column('request_id', sa.Integer(), sa.ForeignKey('requests.id'), primary_key=True),
            sa.Column('checkpoint_id', sa.Integer(), sa.ForeignKey('checkpoints.id'), primary_key=True),
        )
